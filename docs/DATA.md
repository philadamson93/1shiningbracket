...
| M_SIMS | 2000 | Number of simulated tournament outcomes. Each perturbs model probs (via sigma) then plays out all 63 games. More = better coverage of upset scenarios. |
| N_OPPONENTS | 1200 | Opponent brackets per sim, sampled from ESPN public pick distribution. More = smoother position estimates for Kelly EV. Should be >= largest pool field_size. |
| WEALTH_BASE | 0.3 | Kelly log-wealth base. Lower = more diversification across brackets. 0.3 ≈ "I want at least one bracket to cash." |
| N_RESTARTS | 20 | Hill-climb restarts per bracket. Each restart shuffles game traversal order, finding different local optima. Best EV wins. |

### Simulation axes

The precomputation step builds a matrix of `M_SIMS × N_OPPONENTS`:

- **M_SIMS (rows):** Each row is a different "reality" — who actually wins each game.
  Perturbed model probs → simulate 63 games → one tournament outcome. More rows =
  more coverage of upsets, Cinderellas, chalk dominance, etc.

- **N_OPPONENTS (columns):** For each reality, N opponent brackets are generated by
  sampling each game pick from ESPN public pick percentages (247M brackets). Each
  opponent bracket is scored against that reality. The sorted scores tell us where
  our bracket would finish. More columns = smoother position/percentile estimates.

During hill-climbing, every candidate flip is evaluated across all M×N: score the
flipped bracket in each reality, find its position in the opponent distribution,
compute Kelly EV. Both axes matter — skimping on either leaves blind spots.

### Hill-climb restarts

The hill-climber can get stuck in local optima. With deterministic game traversal,
a "Duke champion" bracket might be locally optimal, but "Iowa State champion" could
be globally better — you can't get there by flipping one game at a time.

Restarts solve this: each restart shuffles the order games are evaluated, leading to
different flip sequences and different local optima. Empirically (N=250 pool, 2000 sims):

- 20 restarts found 3 distinct champion clusters (Iowa State, Illinois, Florida)
- EV gap between worst and best restart: **41%** (0.037 → 0.053)
- Best EV first appeared at restart 18 of 20

The first restart uses deterministic order (reproducible baseline); restarts 2-N shuffle.
Cost is ~3.5s per restart per bracket, so 20 restarts × 10 brackets ≈ 12 min.
